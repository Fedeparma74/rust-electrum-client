...
pub trait AsyncElectrumApi {
    /// Gets the block header for height `height`.
    async fn block_header(&self, height: usize) -> Result<BlockHeader, Error> {
        Ok(deserialize(&self.block_header_raw(height).await?)?)
    }

    /// Subscribes to notifications for new block headers, by sending a `blockchain.headers.subscribe` call.
    async fn block_headers_subscribe(&self) -> Result<HeaderNotification, Error> {
        self.block_headers_subscribe_raw().await?.try_into()
    }
    ...
    async fn block_headers_pop(&self) -> Result<Option<HeaderNotification>, Error> {
    ...
    async fn transaction_get(&self, txid: &Txid) -> Result<Transaction, Error> {
    ...
    async fn batch_transaction_get(&self, txids: Vec<Txid>) -> Result<Vec<Transaction>, Error> {
    ...
    async fn batch_block_header(&self, heights: Vec<u32>) -> Result<Vec<BlockHeader>, Error>
    {
    ...
    async fn transaction_broadcast(&self, tx: &Transaction) -> Result<Txid, Error> {
